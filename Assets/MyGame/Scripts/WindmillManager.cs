...
public class WindmillManager : MonoBehaviour
{
    ...
    {
        if (windmills.Length > 0) windmillColor.r = (byte)windmills[0].GetCurrentSpeed();
        if (windmills.Length > 1) windmillColor.g = (byte)windmills[1].GetCurrentSpeed();
        if (windmills.Length > 2) windmillColor.b = (byte)windmills[2].GetCurrentSpeed();
    }
